...
export function Phase1SetupStage({
  ...
  const turnTimerOptionIndex = TURN_TIMER_OPTIONS.findIndex((value) => value === turnTimerSeconds);
  ...
  const canFitFleetLoad = (nextTotalCells: number) => nextTotalCells <= fleetCellLimit;
  const canFitNewShipDraft = (size: number, count: number) => canFitFleetLoad(totalCells + size * count);
  const canAddShipDraft = newShipName.trim().length > 0 && canFitNewShipDraft(newShipSize, newShipCount);
            ...
            {BOARD_PRESETS.map((preset) => {
              ...
              return (
                <button
                  key={preset.label}
                  type='button'
                  onClick={() => onBoardPreset(preset.value.rows, preset.value.cols)}
      ...
      <section className='ui-panel ui-panel-strong themed-scrollbar overflow-visible rounded-md p-5 h-full sm:overflow-y-auto'>
        <div className='relative z-10 flex h-full min-h-0 flex-col gap-4'>
          <div className='flex flex-wrap items-start justify-between gap-3'>
            <div>
              <p className='ui-panel-title'>{t('gameSetup.step1.fleetTitle')}</p>
              <p className='mt-2 text-sm leading-6 text-(--text-muted)'>
              ...
              <div className='ui-subpanel rounded-sm px-4 py-2'>
                ...
                <div className='mt-1 flex items-center gap-2'>
                  <Button
                    ...
                    onClick={() => {
                      onTurnTimerChange(TURN_TIMER_OPTIONS[turnTimerOptionIndex - 1]);
                    }}
                  >
                    -
                  </Button>
          ...
          <div className='themed-scrollbar grid min-h-0 flex-1 gap-1 overflow-y-auto pr-1'>
            {ships.map((ship) => (
              <div
                key={ship.id}
                className='ui-state-idle rounded-sm px-4 py-2'
              >
                <div className='grid gap-3 xl:grid-cols-[minmax(0,1fr)_112px_112px_auto] xl:items-end'>
                  ...
                  <label className='grid gap-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-(--text-muted)'>
                    ...
                    <NumberStepper
                      ...
                      disableIncrease={!canFitFleetLoad(totalCells + ship.count)}
                      onChange={(value) =>
