...
export function OnlineGameHistoryModal({
  ...
  return (
    <Modal
      ...
      surfaceClassName='h-[70vh] min-h-[520px] overflow-hidden'
    >
      {selected ? (
        ...
        <div className='grid h-full min-h-0 gap-3'>
          ...
          {loadState === 'ready' && items.length > 0 && (
            <div className='max-h-[52vh] min-h-0 space-y-3 overflow-y-auto pr-1'>
              {items.map((item) => (
                <button
                  key={item.matchId}
                  type='button'
                  onClick={() => setSelected(item)}
                  className='ui-subpanel block w-full cursor-pointer rounded-sm border border-transparent px-3 py-3 text-left transition-colors hover:border-[rgba(31,136,176,0.35)] hover:bg-[rgba(34,211,238,0.06)]'
                >
