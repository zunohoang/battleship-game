...
async function bootstrap() {
  ...
  const grpcEnabled = isTrue(configService.get<string>('GRPC_ENABLED'), true);
  if (grpcEnabled) {
    app.connectMicroservice<MicroserviceOptions>({
      ...
      options: {
        ...
        url:
          configService.get<string>('GRPC_AUTH_BIND_URL') ?? '0.0.0.0:50051',
        loader: {
          keepCase: true,
          longs: String,
          enums: String,
    ...
    logger.log('gRPC AuthService transport started', 'GRPC');
  }
